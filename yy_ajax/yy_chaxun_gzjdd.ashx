<%@ WebHandler Language="C#" Class="Website.Ajax_chaxun.gzjdd" %>
...
namespace Website.Ajax_chaxun 
{
    ...
    public class gzjdd : IHttpHandler, IRequiresSessionState
    {
       ...
       // string username_c = context.Request.Form["username"];           
           string phone_c=context.Request.Form["phone_chaxun_gzjdd"];
        ...
        else
        {
            ...
            else
            {
               ...
               // string xingmin_zhuanma = HttpUtility.UrlDecode(xingmin_k);
                ...
                string bianma_yy = bianma_time.Trim() + suijima_weiyi.Trim()+"4";
                
                string connStr = ConfigurationManager.ConnectionStrings["yyvodConnectionString"].ToString();
                SqlConnection conn = new SqlConnection(connStr);
                string sql_chaxun_gzjdd = "select bianma_liushui,jindu,time,editor,step,completed from lsjdb where phone=@phone4";
                SqlCommand cmd = new SqlCommand(sql_chaxun_gzjdd, conn);
                ///参数化查询解决非法字符问题，并限制长度，第3层防护                            
                SqlParameter pphone = cmd.Parameters.Add("@phone4",SqlDbType.VarChar,13);             
       ...
       public static bool checkPhone_exist(string phone_e)
       {
           ...
           string connStr = ConfigurationManager.ConnectionStrings["yyvodConnectionString"].ToString();
           SqlConnection conn = new SqlConnection(connStr);
           string sql_exist = "select lkjs,jneirong,phone,steps,completed from lsxxb where phone=@phone3";
