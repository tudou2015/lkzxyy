<%@ WebHandler Language="C#" Class="Website.Ajax_chaxun.ly" %>
...
namespace Website.Ajax_chaxun 
{
    ...
    public class ly : IHttpHandler, IRequiresSessionState
    {
        ...
        else
        {
            ...
            else
            {
               ...
               // string xingmin_zhuanma = HttpUtility.UrlDecode(xingmin_k);
                ...
                string bianma_yy = bianma_time.Trim() + suijima_weiyi.Trim()+"5";
                
                string connStr = ConfigurationManager.ConnectionStrings["yyvodConnectionString"].ToString();
                SqlConnection conn = new SqlConnection(connStr);
                string sql_chaxun_ly = "select lkjs,date,week,sjd,phone from lkzxyy where phone=@phone5";
                SqlCommand cmd = new SqlCommand(sql_chaxun_ly, conn);
       ...
       public static bool checkPhone_exist(string phone_e)
       {
           ...
           string connStr = ConfigurationManager.ConnectionStrings["yyvodConnectionString"].ToString();
           SqlConnection conn = new SqlConnection(connStr);
           string sql_exist = "select lkjs,date,week,sjd,phone from lkzxyy where phone=@phone5";
